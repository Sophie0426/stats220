scrape_search_results <- function(page_path) {
  ...
  map_df(nodes, function(node) {
    ...
    # Date and time
    date_element <- node %>% html_element(".views-field-field-beehive-date, time")
    ...
    # Ministers
    minister <- node %>%
      html_elements(".views-field-field-ministers li a") %>%
      html_text2() %>%
    ...
    # Portfolios
    portfolio <- node %>%
      html_elements(".taxonomy-term--type-portfolios a") %>%
      html_text2() %>%
      # trim out excess white space noise, and collapse them using a semicolon
      str_trim() %>%
      paste(collapse = ";")
